PROGRAM wlMaskTest
  ...
  LOGICAL, DIMENSION(:), ALLOCATABLE :: MaskOption
  ...
  REAL(dp) :: Yemin, Yemax, logrho, logT, logTmin, logTmax, logrhomin, logrhomax, epsilon
 ...
 4018 FORMAT (i4,3(es12.5))
  ...
  ALLOCATE( rho( NumPoints ), T( NumPoints ), Ye( NumPoints ), rand( NumPoints, 3 ),  &
            ...
            Derivative( NumPoints, 3 ), MaskOption( NumPoints),                       & 
  ...
  MaskOption(1:500) = .true. 
  MaskOption(501:1000) = .false. 
    ...
    DO j = 1, EOSTable % DV % nVariables
      CALL LogInterpolateSingleVariable &
             ( rho, T, Ye,                                   &
               ...
               Interpolant, MaskOption )

      WRITE (TestUnit1,*) i, "Interpolant=", Interpolant(i),   &
  ...
  CALL LogInterpolateAllVariables( rho, T, Ye, EOSTable % TS % LogInterp, &
                               EOSTable % TS, EOSTable % DV, Interpolants, MaskOption ) 
  DO i = 1, SIZE(rho)
    DO j = 1, EOSTable % DV % nVariables 
  ...
  DO i = 1, EOSTable % DV % nVariables
    CALL LogInterpolateDifferentiateSingleVariable( rho, T, Ye,        &
                         ...
                         Interpolant(:), Derivative(:,:), MaskOption )
  ...
  CALL LogInterpolateDifferentiateAllVariables( rho, T, Ye, EOSTable % TS % LogInterp, EOSTable % TS, EOSTable % DV, Interpolants, Derivatives, MaskOption ) 
