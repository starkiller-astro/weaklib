  ...
  DO iD=1,SizeDs
    DO iT=1,SizeTs
      DO iYp=1,SizeYps
        ...
        ElectronPhotonGasState % t = Ts_bary(iT)
        ElectronPhotonGasState % rho = Ds_bary(iD)
        ElectronPhotonGasState % ye = Yps_bary(iYp) - Ymu_temp
        
        ! calculate electron quantities
        CALL ElectronPhotonEOS(HelmTable, ElectronPhotonGasState)

        Es_helm = ElectronPhotonGasState % e
        Ps_helm = ElectronPhotonGasState % p
        Ss_helm = ElectronPhotonGasState % s

        ! calculate muon quantities 
        MuonGasState % t = Ts_bary(iT)
        MuonGasState % rho   = Ds_bary(iD)
        MuonGasState % rhoym = MuonGasState % rho * Ymu_temp
        
        CALL FullMuonEOS(MuonTable, MuonGasState)

        Es_muon = MuonGasState % e
        Ps_muon = MuonGasState % p
        Ss_muon = MuonGasState % s
...
! #endif
  ...
  DO iP = 1, nPoints
    ...
    ! Now add electron component
    ! Initialize temperature, DensitY, Ye
    ElectronPhotonGasState % t = T(iP)
    ElectronPhotonGasState % rho = D(iP)
    ElectronPhotonGasState % ye = Ye(iP)
    ...
    ! calculate muon quantities 
    MuonGasState % t = T(iP)
    MuonGasState % rho   = D(iP)
    MuonGasState % rhoym = MuonGasState % rho * Ym(iP)
