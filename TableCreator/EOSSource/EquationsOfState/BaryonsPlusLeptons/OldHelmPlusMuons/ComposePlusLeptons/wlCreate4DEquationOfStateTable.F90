PROGRAM wlCreateEquationOfStateTable
    ...
    USE wlLeptonEOSTableModule, ONLY: HelmTableType, MuonTableType, &
        iTempMax, iDenMax, &
        nTempMuon, nDenMuon, &
    ...
    iCount = 0
    !$OMP PARALLEL DO PRIVATE(ElectronPhotonGasState, MuonGasState, Ye, Ym, Yp, iYp, dYp, LocalOffset, Xbary) &
    !$OMP SHARED(iCount) &
    !$OMP COLLAPSE(3)
    DO iRho=1,nPoints(1)
      DO iTemp=1,nPoints(2)
        DO iYe=1,nPoints(3)-1
          DO iYm=1,nPoints(4)
            ...
            ElectronPhotonGasState % rho = EOSTable % TS % States(1) % Values(iRho)
            ElectronPhotonGasState % t   = EOSTable % TS % States(2) % Values(iTemp)
            ...
            MuonGasState % rhoym = MuonGasState % rho * Ym
            ...
            Xbary + MuonGasState % p + ElectronPhotonGasState % p

            IF (EOSTable % DV % Variables(1) % Values(iRho,iTemp,iYe,iYm) < 0.0d0) THEN
              WRITE(*,*) iYe, iYp, Ye, Ym, Yp, dYp, Xbary, MuonGasState % p, ElectronPhotonGasState % p
              STOP
            ENDIF
            ...
            EOSTable % DV % Variables(2) % Values(iRho,iTemp,iYe,iYm) = &
              Xbary + MuonGasState % s + ElectronPhotonGasState % s
            ...
            EOSTable % DV % Variables(3) % Values(iRho,iTemp,iYe,iYm) = &
              Xbary + MuonGasState % e + ElectronPhotonGasState % e + Add_to_energy

            ! ELECTRON CHEMICAL POTENTIAL
            EOSTable % DV % Variables(4) % Values(iRho,iTemp,iYe,iYm) = &
              ElectronPhotonGasState % mue
    ...
    DO iRho=1,nPoints(1)
      DO iTemp=1,nPoints(2)
        DO iYm=1,nPoints(4)
          ...
          ElectronPhotonGasState % rho = EOSTable % TS % States(1) % Values(iRho)
          ElectronPhotonGasState % t   = EOSTable % TS % States(2) % Values(iTemp)
          ElectronPhotonGasState % ye  = EOSTable % TS % States(3) % Values(nPoints(3))
          CALL ElectronPhotonEOS(HelmTable, ElectronPhotonGasState)

          EOSTable % DV % Variables(1) % Values(iRho,iTemp,nPoints(3),iYm) = &
            EOSCompOSE(iRho,iTemp,nPoints(3),1) + ElectronPhotonGasState % p
          EOSTable % DV % Variables(2) % Values(iRho,iTemp,nPoints(3),iYm) = &
            EOSCompOSE(iRho,iTemp,nPoints(3),2) + ElectronPhotonGasState % s
          EOSTable % DV % Variables(3) % Values(iRho,iTemp,nPoints(3),iYm) = &
            EOSCompOSE(iRho,iTemp,nPoints(3),3) + ElectronPhotonGasState % e
