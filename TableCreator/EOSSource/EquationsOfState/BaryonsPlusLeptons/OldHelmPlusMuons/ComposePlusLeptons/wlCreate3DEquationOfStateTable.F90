PROGRAM wlCreateEquationOfStateTable
    ...
    iCount = 0
    !$OMP PARALLEL DO PRIVATE(ElectronPhotonGasState, Ye) &
    !$OMP SHARED(iCount) &
    !$OMP COLLAPSE(3)
    DO iRho=1,nPoints(1)
      DO iTemp=1,nPoints(2)
        DO iYe=1,nPoints(3)
          ...
          EOSCompOSE(iRho,iTemp,iYe,1) + ElectronPhotonGasState % p

          ! ENTROPY
          EOSTable % DV % Variables(2) % Values(iRho,iTemp,iYe) = &
          EOSCompOSE(iRho,iTemp,iYe,2) + ElectronPhotonGasState % s

          ! ENERGY
          EOSTable % DV % Variables(3) % Values(iRho,iTemp,iYe) = &
          EOSCompOSE(iRho,iTemp,iYe,3) + ElectronPhotonGasState % e + Add_to_energy
