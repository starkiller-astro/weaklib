PROGRAM wlEosInversionTest
  ...
  BaryonPlusHelmTableName = 'BaryonsPlusHelmEOS.h5'
    ...
    DO iRho=1,EOSBaryonTable % DV % nPoints(1)
      DO iTemp=1,EOSBaryonTable % DV % nPoints(2)
        DO iYp=1,EOSBaryonTable % DV % nPoints(3)
          ...
          ElectronState % y_e = Ys_T(iYp)
          
          ! calculate electron quantities
          CALL FullHelmEOS(1, HelmholtzTable, ElectronState, .false., .false.)

          Es_T_helm = ElectronState % e + 0.511 / rmu * ergmev * ElectronState % y_e ! add back mass to internal energy!
          Ps_T_helm = ElectronState % p
          Ss_T_helm = ElectronState % s
          
          Es_T(iRho,iTemp,iYp) = LOG10( 10.0d0**( Es_T(iRho,iTemp,iYp) ) + Es_T_helm)
          Ps_T(iRho,iTemp,iYp) = LOG10( 10.0d0**( Ps_T(iRho,iTemp,iYp) ) + Ps_T_helm)
          Ss_T(iRho,iTemp,iYp) = LOG10( 10.0d0**( Ss_T(iRho,iTemp,iYp) ) + Ss_T_helm)

          ! calculate muon quantities and convert units properly (SHOULD HAVE BEEN DONE WHEN READING IN THE TABLE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!)
          MuonState % t = Ts_T(iTemp)
          MuonState % rho = Ds_T(iRho) * Ymu
          
          CALL FullHelmEOS(MuonTable, MuonState)

          Es_T_muon = MuonState % e ! do you need to add rest mass too? Probably YES
          Ps_T_muon = MuonState % p
          Ss_T_muon = MuonState % s
          
          Es_T(iRho,iTemp,iYp) = LOG10( 10.0d0**( Es_T(iRho,iTemp,iYp) ) + Es_T_muon)
          Ps_T(iRho,iTemp,iYp) = LOG10( 10.0d0**( Ps_T(iRho,iTemp,iYp) ) + Ps_T_muon)
          Ss_T(iRho,iTemp,iYp) = LOG10( 10.0d0**( Ss_T(iRho,iTemp,iYp) ) + Ss_T_muon)
