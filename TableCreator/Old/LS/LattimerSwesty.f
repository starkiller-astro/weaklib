...
C
      INTEGER SSFLAG
      DOUBLE PRECISION INPVAR(4),YE,BRYDNS,P_PREV,T
C
C                       "ZERO" flag
      INTEGER ZFLAG
C
...
C
c      DATA X /.087649410479D00
c     *,.46269632892D00,.11410577748D01,.21292836451D01,.34370866339D01
c     *,.50780186145D01,.70703385350D01,.94383143364D01,.12214223369D02
c     *,.15441527369D02,.19180156857D02,.23515905694D02,.28578729743D02
c     *,.34583398702D02,.41940452648D02,.51701160340D02/
c      DATA WGHT /.22503631486D00
c     *,.52583605276D00,.83196139169D00,.11460992410D01,.14717513170D01
c     *,.18131346874D01,.21755175197D01,.25657627502D01,.29932150864D01
c     *,.34712344831D01,.40200440864D01,.46725166077D01,.54874206580D01
c     *,.65853612333D01,.82763579844D01,.11824277552D02/
C
C23456789012345678901234567890123456789012345678901234567890123456789012
      DATA X /
     * 3.961320640860D-02 ,  2.088002856543D-01 ,  5.135107755352D-01 ,
     * 9.543811530791D-01 ,  1.532249225394D+00 ,  2.248215817854D+00 ,
     * 3.103661490498D+00 ,  4.100262545699D+00 ,  5.240010108755D+00 ,
     * 6.525233330468D+00 ,  7.958627508999D+00 ,  9.543288035279D+00 ,
     * 1.128275128666D+01 ,  1.318104390025D+01 ,  1.524274226656D+01 ,
     * 1.747304463141D+01 ,  1.987785893308D+01 ,  2.246391051439D+01 ,
     * 2.523887525730D+01 ,  2.821154567688D+01 ,  3.139204037421D+01 ,
     * 3.479207144800D+01 ,  3.842529076480D+01 ,  4.230774567208D+01 ,
     * 4.645849004290D+01 ,  5.090042150332D+01 ,  5.566145791468D+01 ,
     * 6.077624068453D+01 ,  6.628869068025D+01 ,  7.225601475446D+01 ,
     * 7.875533816278D+01 ,  8.589548142950D+01 ,  9.383992978484D+01 ,
     * 1.028580101471D+02 ,  1.134651354897D+02 ,  1.269880151966D+02 /
C
      DATA WGHT /
     * 1.016694751217D-01 ,  2.368502055548D-01 ,  3.726715786250D-01 ,
     * 5.092083225550D-01 ,  6.467110475491D-01 ,  7.854519690232D-01 ,
     * 9.257184440339D-01 ,  1.067815135684D+00 ,  1.212067715646D+00 ,
     * 1.358827381029D+00 ,  1.508476196330D+00 ,  1.661433435563D+00 ,
     * 1.818163187438D+00 ,  1.979183577131D+00 ,  2.145078075547D+00 ,
     * 2.316509528323D+00 ,  2.494237764181D+00 ,  2.679141968116D+00 ,
     * 2.872249479966D+00 ,  3.074773384330D+00 ,  3.288162326323D+00 ,
     * 3.514167642788D+00 ,  3.754935526960D+00 ,  4.013136236755D+00 ,
     * 4.292149591579D+00 ,  4.596338646060D+00 ,  4.931466465190D+00 ,
     * 5.305354988656D+00 ,  5.728974249695D+00 ,  6.218344086884D+00 ,
     * 6.798089006814D+00 ,  7.508695084046D+00 ,  8.423188762282D+00 ,
     * 9.692562072805D+00 ,  1.170765394670D+01 ,  1.607328906573D+01 /
C
C
C-----------------------------------------------------------------------
C
...
C                            Calculate the new change in ETA
        D_ETA = -(NE_CHK-NSUBE)/(DNENDE*TFAC+EPSIL)
C
...
C23456789012345678901234567890123456789012345678901234567890123456789012
C
      SUBROUTINE EOS_M4C(INPVAR,YE,BRYDNS,IFLAG,EOSFLG,FFLAG,
     1                   SSFLAG,XPREV,P_PREV)
C
      USE eos_m4c_module, ONLY: SWTFLG, RSFLAG, XH, HEAVCT, NF_FLG
...
C
      INTEGER IFLAG,EOSFLG,FFLAG,SSFLAG
      DOUBLE PRECISION INPVAR(4),YE,BRYDNS,XPREV,P_PREV
      DOUBLE PRECISION T
C
C
...
C
      INTEGER EOSFLG
      DOUBLE PRECISION INPVAR(4),YE,BRYDNS,T
C
C
 10   CONTINUE
C
...
C
        ...
        IF ( I_MXWL == 0 ) THEN
          WRITE (*,3001) I_MXWL, T_HI, T_LOW, T
 3001     FORMAT (' i_MXWL=',i4,' T_HI=',es11.3,' T_LOW=',es11.3,
     1     ' T=',es11.3)
        ENDIF
C
C
...
CC      D_MXWL = YINTRP*(D_H(J_MXWL+1)-D_H(J_MXWL))+D_H(J_MXWL)
        ...
        IF ( J_MXWL == 0 ) THEN
          WRITE (*,3002) J_MXWL, T_HI, T_LOW, T
 3002     FORMAT (' J_MXWL=',i4,' T_HI=',es11.3,' T_LOW=',es11.3,
     1     ' T=',es11.3)
        ENDIF
...
C                       Ye is too low
      ELSE
        ...
        WRITE(*,"(' rho, T, Ye=',3es11.3)") 
     & 1.67d+15 * BRYDNS, T/8.62d-11,YE
        YE = Y_LOW+1.0D-6
        GOTO 10
      ENDIF
C
...
C
      INTEGER SSFLAG
      DOUBLE PRECISION INPVAR(4),YE,BRYDNS,XPREV,P_PREV,BRYDNS_T,T
C
C
...
C
      INTEGER SSFLAG
      DOUBLE PRECISION INPVAR(4),YE,BRYDNS,XPREV,P_PREV,T
...
C23456789012345678901234567890123456789012345678901234567890123456789012
C
      SUBROUTINE SILEOS(MODE,T,YE,BRYDNS,EB)
C
      USE el_eos_module
      USE eos_m4c_module
C
