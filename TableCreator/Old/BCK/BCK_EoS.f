...
c***********************************************************************
      parameter(third=1./3.,ba=-7.073915,zero=0.0,dnp=1.29333)
      parameter(a0=.067,c0=2.37745e-4)
c                                                                      c
c***********************************************************************
...
c******************************************** get energy zeroes ********
      ...
      write(*,999)yeFe,d00*theta0*fphi(yeFe),theta0,size0,egy0
      first = .false.
             endif
c***********************************************************************
                        return
...
c                                                                      c
      ...
      parameter(bsil=-8.447744,ba=-7.073915)
      parameter(a0=.067,c0=2.37745e-4,piby2=1.570796327)
      common /tcrit/tc
c***********************************************************************
      f3(z) = sign( (abs(z))**third , z)
...
c*********************************************************************
     ...
     *        + (dbdlph-dbdlth)*dlphix
     ...
     &        + fbxthe(theta)
c***********************************************************************
      b_hvy   = fbulk(zabck) + wsize - excit*xmstar + fcomp(theta)
      tol1    = (dbdlu - dbdlth + tsi*tbck*dbck/d0)
     *        / (fxk(zabck)/9.*theta)
      tol2    = dbdx + uhat
      tol3    = yebck -  (2.0 * ya + ypbck)*u1pack - zabck * xhbck
      tol4    = -un + zabck*uhat + b_hvy + dbdlth + uhtra
c***********************************************************************
              return
c***********************************************************************
c***********************************************************************
...
c***********************************************************************
      ...
      save
c                                                                      c
      parameter(nuc=6,zero=0.0,const=2.37745e-4,ncnvge=20,dnp=1.29333)
...
c                                                                      c
      ...
      data zan/1.,0.,.5,.4642857,.5,.5/,zza/1.,0.,1.,12.07142857,7.,4./
      data bn/0.,0.,-7.073915,-8.790323,-8.447744,-7.976206/
      data betem/0.,0.,-7.073915,-8.790323,-8.447744,-7.976206/
...
c Change made on 10/18/05 by SWB
      b = bes
      b_hvy = ( bes - x(3)*bn(3) )/( dabs( xhbck ) + 1.d-100 )
...
c                                                                      c
      parameter(xms=.7,xm0=2.,xm0ms=xm0-xms)
      parameter(ba=-7.073915)
      parameter(a0=.067,c0=2.37745e-4)
      parameter(ncnvge = 10,epsf=4.e-7,epsg=4.e-8,epsh=4.e-7,epse=4.e-8)
      parameter (bsil = -8.447744)
      parameter (tenm3=1.e-3,dzamin=1.e-12)
c                                                                      c
...
c***********************************************************************
     ...
     & .or. (iters.ge. ncnvge .and. bad) ) then
                        dtran = dbck
                        write(*,993)dtran,upack,d0,theta,xhbck
                        call eosnm
                        return
                 endif
                 endif
  ...
  103 FORMAT (' Failure in Saha: rho=',es11.3,
     * ' t=',es11.3,' ye=',es11.3)
      if( .not. fmbad  .and.  dbck * rmu/cm3fm3 .le. 1.d+11 ) then
        ...
        return
      end if ! .not. fmbad  .and.  dbck * rmu/cm3fm3 .le. 1.d+11
c******************************************************failure?*********
                        dtran = dbck
                        write(*,993)dtran,upack,d0,theta,xhbck
                        call eosnm
...
!      sh = 0.d0
     ...
     * * (2.*xmstar-2.*tbck/tc/(1.+tbck/tc)*(xmstar-xms) )
      eh = xhbck * b_hvy + xabck*ba + tbck*sh
      b  = xhbck * b_hvy + xabck * ba
!      eh = b
      return
c***********************************************************************
993   format(' set dtran=',f8.5,'  upack=',f7.3,'  d0=',f8.5,
