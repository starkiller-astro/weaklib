PROGRAM wlTestParallel4DGSI
  ...
  t_4D_serial = REAL(count_end1 - count_start) / REAL(count_rate)
 ...
 !$omp END PARALLEL DO
  ...
  t_4D = REAL(count_end2 - count_end1) / REAL(count_rate)
  t_tot = REAL(count_end2 - count_start) / REAL(count_rate)
   ...
   WRITE(*,'(A,f10.3)')   ' Ratio 4D     :', t_4D_serial/t_4D
