...
module CC_global
  ...
  real*8:: Enu,En,pnmax,pnmin,ppmax,pemax,pemin,Tfac=150.0d0
  integer:: anti,opt0,opt_form=0,opt_pseudo=0,opt_WM=0
  real*8:: massA=1.d3,massV=840.0d0,Mpi=139.57d0
...
subroutine Integral_2D(xEnu,res)   ! captures
  ...
  res = res*(gf*Vud)**2/16.0d0/(pi**5)/197.327d-18
12 continue
...
subroutine Calc_Ampsq( E2, E3, ampsq )
  ...
  real*8:: gasq=0.0d0,gvsq=0.0d0,gva=0.0d0,gaf=0.0d0,gvf=0.0d0,f2sq=0.0d0
...
!!! adding form-factor dependences
  ...
  if(opt_form .eq. 1) then
     VmassSq = 1.0d0/(1.0d0/massV**2-3.706d0/m2**2/8.0d0)
     AVmassSq = 1.0d0/(0.5d0/massA**2+0.5d0/massV**2-3.706d0/m2**2/16.0d0)
     AFmassSq = 1.0d0/(0.5d0/massA**2+0.5d0/massV**2+1.0d0/m2**2/16.0d0)
     VFmassSq = 1.0d0/(1.0d0/massV**2-(3.706d0-1.0d0)/16.0d0/m2**2)
     ...
     Ampsq_ff0 = cplA2*Ia2 + cplB2*Ib2 + cplC2*Ic2 + cplD2*Id2 + &
          ...
          cplL2*Il2 
     if(Ampsq+ampsq_ff0.ge.0.0d0) Ampsq=Ampsq+ampsq_ff0           
  end if
...
!!! adding pseudoscalar terms w/o form factor dependence
  if(opt_pseudo .eq. 1) then
	...
	if( ZAP>0.0d0) then
     ...
     if(Ampsq+ampsq_pseudo.ge.0.0d0) Ampsq=Ampsq+ampsq_pseudo
  end if
...
subroutine Ebounds( xpn3, xq0_min, xq0_max )
  ...
  if( xB**2 - 4.0d0*xA*xC .ge. 0.0d0) then
...
subroutine Ebounds_D( xpn3, xq0_min, xq0_max )
  ...
  if( abs(xq0_max-xq0_min)<1.d-15 ) then
     if(opt0.eq.3) then
	xq0_max = max(Enu + mue + 50.0d0*Tem, xq0_min + 50.0d0*Tem)
     else if(opt0.eq.4) then
	xq0_max = max(xq0_min+50.0d0*Tem, sqrt(ppmax**2+mp**2)+Up-En)
     end if
  end if
