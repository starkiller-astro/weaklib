...
MODULE wlSemiLeptonicOpacityModule2D
  ...
  REAL(DP), PARAMETER :: ROOT_REL_TOL      = 1.0d-8
  REAL(DP), PARAMETER :: SMALL_Q0_SPAN_TOL = 1.0d-15
  REAL(DP), PARAMETER :: Q0_FALLBACK_TFAC  = 50.0d0
...
SUBROUTINE Opacity_CC_2D(WhichCorrection, ReactionIndex, Enu, OpaA, xT, xMul, xMun, xMup, &
  ...
  CASE DEFAULT
     PRINT *, 'Error: Invalid WhichCorrection value in Opacity_CC_2D: ', WhichCorrection
     STOP
  END SELECT
...
SUBROUTINE Calc_Ampsq( Enu, T, E2, E3, Mass2, Mass3, Mass4, U2, U4, anti, ReactionIndex, IncludeCorrections, Ampsq )
  ...
  raw_ampsq = cplA*Ia + cplB*Ib + cplC*Ic + cplD*Id + cplE*Ie + cplG*Ig + cplH*Ih + &
...
SUBROUTINE Ebounds( E2, Enu, T, P2, Mass2, Mass3, Mass4, U2, U4, xq0_min, xq0_max )
  ...
  DO i=1,4
    ...
    IF( ABS((tmp1-tmp3)/tmp3)<ROOT_REL_TOL .or. ABS((tmp1-tmp4)/tmp4)<ROOT_REL_TOL ) THEN
      xq0 = tmp1
    END IF
    IF( ABS((tmp2-tmp3)/tmp3)<ROOT_REL_TOL .or. ABS((tmp2-tmp4)/tmp4)<ROOT_REL_TOL ) THEN
      xq0 = tmp2
    END IF
...
SUBROUTINE Ebounds_D( E2, Enu, T, P2, Mass2, Mass3, Mass4, U2, U4, Mu3, p4max, ReactionIndex, xq0_min, xq0_max )
  ...
  DO i=1,4
    ...
    xq0_max = max( xq0,  xq0_max)
  END DO
  ! ! Inside Ebounds_D, replace the DO i=1,4 loop inner logic with:
  ! DO i=1,4
  !   xq3 = q3_lim(i)
  !   tmp1 = Enu + SQRT( (Enu+xq3)**2 + Mass3**2 )
  !   tmp2 = Enu + SQRT( (Enu-xq3)**2 + Mass3**2 )
  ...
  !   IF( ABS((tmp1-tmp3)/tmp3)<ROOT_REL_TOL .or. ABS((tmp1-tmp4)/tmp4)<ROOT_REL_TOL ) THEN
  !     xq0_min = min( tmp1,  xq0_min)
  !     xq0_max = max( tmp1,  xq0_max)
  !   END IF
  !   IF( ABS((tmp2-tmp3)/tmp3)<ROOT_REL_TOL .or. ABS((tmp2-tmp4)/tmp4)<ROOT_REL_TOL ) THEN
  !     xq0_min = min( tmp2,  xq0_min)
  !     xq0_max = max( tmp2,  xq0_max)
  !   END IF
  ! END DO
  IF( ABS(xq0_max-xq0_min) < SMALL_Q0_SPAN_TOL ) THEN
    IF(ReactionIndex.eq.3) THEN
      xq0_max = max(Enu + Mu3 + Q0_FALLBACK_TFAC*T, xq0_min + Q0_FALLBACK_TFAC*T)
    ELSE IF(ReactionIndex.eq.4) THEN
      xq0_max = max(xq0_min + Q0_FALLBACK_TFAC*T, SQRT(p4max**2+Mass4**2)+U4-E2)
    END IF
  END IF
